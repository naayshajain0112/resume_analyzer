...
function App() {
  ...
  const handleDownloadReport = () => {
    const reportText = apiResponse?.analysis || apiResponse?.response || '';
    const cvFileName = submitDetails?.cvFileName || 'report';
    // Strip extension from CV filename and append _analysis.pdf
    const baseName = cvFileName.replace(/\.[^/.]+$/, '');
    const downloadName = `${baseName}_analysis.pdf`;

    // Build a styled HTML page and open it in a new window for print-to-PDF
    const htmlContent = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <title>${baseName} Analysis Report</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .meta span { color: #334155; font-weight: 600; }
    .report-body {
      font-size: 14px;
    ...
    @media print {
      ...
      .report-body { background: #fff; border: none; padding: 0; }
    }
  </style>
</head>
<body>
  <div class="header">
    <div class="badge">CV Analysis Report</div>
    <h1>CV vs. LinkedIn Consistency Report</h1>
    <div class="meta">
      CV File: <span>${cvFileName}</span>&nbsp;&nbsp;|&nbsp;&nbsp;
      LinkedIn: <span>${submitDetails?.linkedinUrl || 'N/A'}</span>&nbsp;&nbsp;|&nbsp;&nbsp;
      Generated: <span>${new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
    </div>
  </div>
  <div class="report-body">${reportText.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</div>
  <div class="footer">Generated by Resume Analyser &mdash; Powered by Gemini 1.5 Pro</div>
  <script>
    window.onload = function() {
      document.title = ${JSON.stringify(downloadName)};
      window.print();
    };
  <\/script>
</body>
    ...
    // Clean up the object URL after the window is done with it
    if (printWindow) {
          ...
          <div className="w-full max-w-4xl mx-auto space-y-6 animate-[fadeIn_0.5s_ease-out]">
            ...
            <div className="flex gap-4 justify-center flex-wrap">
              ...
              <button
                ...
                className="px-6 py-3 rounded-xl text-white font-semibold bg-emerald-600 hover:bg-emerald-500 transition-all duration-300 shadow-lg hover:shadow-emerald-500/20 active:scale-[0.98] flex items-center gap-2"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3M3 17a9 9 0 0018 0M3 17V7a9 9 0 0118 0v10" />
                </svg>
                Download Report
              </button>
              <button
